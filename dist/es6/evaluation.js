(function (global, factory) {
  ...
  var Button = /*#__PURE__*/function () {
    function Button(onClick, _ref) {
      var _ref$label = _ref.label,
          label = _ref$label === void 0 ? "button" : _ref$label,
          _ref$hoverText = _ref.hoverText,
          hoverText = _ref$hoverText === void 0 ? "button" : _ref$hoverText,
          _ref$optionalID = _ref.optionalID,
          optionalID = _ref$optionalID === void 0 ? null : _ref$optionalID,
          _ref$buttonClassName = _ref.buttonClassName,
          buttonClassName = _ref$buttonClassName === void 0 ? "button--alternate" : _ref$buttonClassName,
          ...
          onMouseOver = _ref$onMouseOver === void 0 ? function (e) {} : _ref$onMouseOver,
          _ref$sideEffect = _ref.sideEffect,
          sideEffect = _ref$sideEffect === void 0 ? function (e) {} : _ref$sideEffect;

      _classCallCheck(this, Button);

      // Create this object's HTML template.
      this.template = html(_templateObject$3 || (_templateObject$3 = _taggedTemplateLiteral(["\n            <label class=\"", "\"\">\n                <button\n                    class=\"", " button--alternate\"\n                    title=\"", "\"\n                    type=\"button\"\n                    id=\"", "\"\n                    @click=\"", "\"\n                    @mouseover=\"", "\"\n                >\n                    ", "\n                </button>\n            </label>\n        "])), labelClassName, buttonClassName, hoverText, optionalID, function (e) {
        onClick(e);
        sideEffect(e);
      }, function (e) {
        return onMouseOver(e);
      }, label); // Get the internal object and make it accessible to the user in the
      // event they need to modify it.

      this.entity = document.getElementById(optionalID);
      this.state = state; // Return the TemplateResult so it can be rendered to the screen.
  ...
  var PlanUploader = /*#__PURE__*/function () {
  ...
  ({
    multimember: html(_templateObject$1 || (_templateObject$1 = _taggedTemplateLiteral(["\n        <div class=\"place-info\">\n            Multi-member districts of varying sizes\n        </div>\n    "]))),
    community: html(_templateObject2 || (_templateObject2 = _taggedTemplateLiteral(["\n        <div class=\"place-info\">Identify a community</div>\n    "])))
  });
  ...
  geoPath(geoAlbersUsaTerritories$1.geoAlbersUsaTerritories().scale(scale).translate(translate)).pointRadius(2);
  ({
  ...
  var evaluation = (function () {
    fetch("/assets/data/landing_pages.json?v=2").then(function (response) {
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var st = _step.value;
          var ref = uspost[st.state];

          var _iterator2 = _createForOfIteratorHelper(st.sections),
              _step2;
      ...
      var go = new Button(function () {
        var url = document.getElementById("shareable-url").value;
        var plan = loadPlan(url);
        plan.then(function (context) {
          savePlanToStorage(context);
          navigateTo("/eval");
        });
      }, {
        label: "Go.",
        hoverText: "Evaluate the selected plan."
      });
      var upload = new PlanUploader(function (fileContent) {
        loadPlanFromJSON(JSON.parse(fileContent)).then(function (context) {
          savePlanToStorage(context);
          navigateTo("/eval");
        });
      });
      var from_link = html(_templateObject || (_templateObject = _taggedTemplateLiteral(["<div class=\"modal-item\" id=\"url\">\n                    <p>\n                        Paste the shareable URL or plan code for an existing Districtr\n                        plan.\n                    </p>\n                    <input type=\"url\" id=\"shareable-url\">\n                    <div class=\"go-button\">", "</div>\n                </div>"])), go);
      render(from_link, document.getElementById('link-upload'));
      render(upload.render(), document.getElementById("json-upload")); // Build map for clicking for loadable plans
  ...
  function loadPlan(url) {
    ...
    if (window.location.href.includes("localhost:") && url == "") return loadPlanFromURL("/assets/mi-plans/state_house.json?v=2");
    var districtr_id = url.split('/')[url.split('/').length - 1];
    return fetch('https://districtr.org/.netlify/functions/planRead?id=' + districtr_id).then(function (res) {
      return res.json();
    }).then(loadPlanFromJSON);
  }
